...
function useTopupReturn() {
  ...
  useEffect(() => {
    if (!outcome) return;
    if (outcome === "paid") {
      toast.success("Payment received. Your funds appear in a moment.");
      refresh();
    }
  ...
  return (
    <div className="space-y-4">
      ...
      {!query.isLoading && items.length === 0 && (
        <EmptyState
          icon={<Coins />}
          title="Nothing in your wallet yet"
          description="Funds appear here once you add funds or an ad runs."
        />
      )}
