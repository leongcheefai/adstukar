...
/**
 * The expanded view of one placement. A CapyTV placement opens its player; a
 * Snippet placement opens its code and its rules. The two never share a screen,
 * because they are two different products wearing one word.
 */
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-h-[85vh] overflow-y-auto sm:max-w-xl">
        <DialogHeader>
          <DialogTitle>{mode === "tv" ? "CapyTV" : "Snippet"}</DialogTitle>
          <DialogDescription>
            {mode === "tv" ? (
              "Plays a full-screen spot from the advertiser pool. No code to install."
            ) : (
              <>
                Shows random ads from the advertiser pool on{" "}
            ...
            <div className="grid gap-6 sm:grid-cols-2">
              ...
              <div className="space-y-1.5">
                ...
                <p className="text-xs text-muted-foreground">
                  Share of impressions that show your own product instead. Earns nothing, costs
                  nothing.
                </p>
              </div>
            </div>
        ...
        {/* Delete sits under a rule at the bottom of both modes: it is the one
            action that is not about setting this placement up. */}
