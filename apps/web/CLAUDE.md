...
- Economy numbers shown on the site come from `@repo/config/economy` — never type an amount. The play rate is tier by format, so the copy derives a range from `economy.playRate` rather than quoting one number
