...
| rate | what a screen earns for one play, by tier: `economy.earn`. No fee comes off it |
...
- **The pool** — slot revenue is capped at the ring, and the earn grows with every screen approved. Nothing in code stops a play from paying when the pool is spent. `GET /admin/pool` shows the week's revenue beside the week's earn, and an admin moves one of three levers by hand: the slot price, the pace of approval, or the pace of tier promotion (`docs/adr/0010`).
