...
## Purpose
CapyAds is an ad network for small screens. An advertiser pays to have listings played on screens that other members own. The screen owner earns from each play, and cashes out what they earned. Money enters as a top-up and leaves as a payout; the ledger holds one integer unit, one thousandth of a US dollar, and every page shows dollars (`docs/adr/0007`). Built on the Vite + Hono + Better Auth boilerplate: pnpm + Turborepo monorepo with five apps (`web` landing, `app` dashboard, `api`, `capytv` screen, `embed` snippet) and seven shared packages (`ui`, `db`, `auth`, `emails`, `env`, `config`, `contracts`). Vocabulary: `CONTEXT.md`. Decisions: `docs/adr/`. Roadmap: GitHub issue #5.
...
The old barter economy (+1 earn, -2 spend, no money) is dead, and so is the
per-play advertiser bill on venue screens (`docs/adr/0010`). Do not restore either.

| Term | What it is |
|---|---|
| `campaign` | one destination site, its verified domain, its state |
| `listing` | one creative under a campaign, up to four |
| `device` | one screen running CapyTV; owner, venue, location, daily play cap |
| `placement` | one overlay region on a device (`band` / `float` / `ticker`) |
| `play` | one listing shown in one placement for its dwell. The platform pays the screen a fixed rate for it |
| rate | what a screen earns for one play, the same on every screen: `economy.earn`. No fee comes off it |
...
- **Nothing is priced when a play is served** — the daily cap, the paid hours, the state of the slot, the campaign, and the listing are all read at report time. Above the cap, after the paid hours, or on a creative an admin rejected after the batch was cut, the play still shows and still counts, and simply pays nothing. A scan is recorded and never paid.
- **Slots** — `apps/api/src/modules/slots/`: `slots.ts` holds the pure rules (what refunds, the ring, availability; the price and the term end live in `@repo/config/economy`), `term.ts` starts, refunds and ends a term inside the transaction of the act that moved it (a rejection refunds only a `booked` slot; an archive also ends a running one), and `slots.service.ts` books in one transaction: campaign, creative, charge, slot. The charge is a `spend` with no `playId`. The platform pays every venue screen that plays the slot, from this charge (`docs/adr/0010`). Only `booked` refunds. A slot does not pause: `updateCampaign` and `updateListing` refuse `paused` while `hasLiveSlot` is true, and the dashboard shows no pause control. `GET /slots/loop` needs no session and names no member; it stays on the `APP_URL` allow-list, because only the dashboard's own set prints it. See `docs/adr/0009`.
- **The pool** — slot revenue is capped at the ring, and the earn grows with every screen approved. Nothing in code stops a play from paying when the pool is spent. `GET /admin/pool` shows the week's revenue beside the week's earn, and an admin moves one of two levers by hand: the slot price or the pace of approval (`docs/adr/0010`, `docs/adr/0013`).
- **One paid listing at a time** — a device may hold several placements, but only one paid listing is on screen at once. Concurrent regions would post several earns for one pair of eyes.
...
- **Payout** — `apps/api/src/modules/payouts/`: `eligibility.ts` holds the rules
  ...
  amount comes back. A distributor connects a Stripe account through Stripe's hosted onboarding; `stripe_account.payouts_enabled` is the only flag that gates a request. Approval sends a Stripe Transfer keyed on the request id, inside the transaction that marks the row paid. The Transfer is in MYR: the platform is a Malaysia account and settles MYR only, so `payPayout` converts the dollars once, at Bank Negara's rate for the day (`fx.ts` pure, `bnm.ts` fetches), refuses a rate outside `economy.payout.paidIn.rateBounds`, and stamps `paid_cents`, `paid_currency`, `fx_rate` and `fx_rate_date` from the transfer. See `docs/adr/0005`, `docs/adr/0008`, `docs/adr/0011` and `docs/adr/0012`.
- **Moderation** — `apps/api/src/modules/admin/`: a human queue. An admin reviews each listing and each device, and a device carries a photo of the screen in place. Every approved device earns the one rate; there is no tier (`docs/adr/0013`). The domain check stays automatic and gates the campaign.
...
- **Fraud is bounded by policy, not by hardware** — CapyTV is a PWA, so there is no device attestation. Approval, the daily play cap, the paid hours, the payout hold, and the scan-to-play ratio are the whole defence. See `docs/adr/0003`. The play cap bounds the money and the paid hours bound the time: a device's paid window opens at its first play of the UTC day and closes `economy.caps.paidHoursPerDay` later, so a screen that plays all night is paid for none of it. CapyTV stops its play cycle while the page is hidden and owes nothing for it; the server cannot verify that, so it is the honest-client half. The payout review reads three signals per screen: the scan-to-play ratio, the plays that fell outside the venue's stated open hours, and whether devices share an address or a network. `device.last_network` holds a prefix, never an address, and the stated hours are read in `device.timezone`.
- **Jobs** — `apps/api/src/modules/jobs/`: settlement (pending → settled), expiry, and the void of an open play whose report never came. A fourth job ends a slot whose term is over. The jobs run in-process (`JOBS_ENABLED=true`), or once with `pnpm --filter @repo/api jobs:run`.
...
- **Presets** — `apps/api/src/modules/presets/`: pictures and clips an admin puts at the front of every member's Images/Video library (`preset_media`). Any member reads `GET /presets`; only an admin presigns (`POST /admin/presets/presign`, the `presets/` prefix, the caps in `@repo/config/media`), records (`POST /admin/presets`), renames (`PATCH`) or deletes (`DELETE /admin/presets/:id`). The record takes only the key and a name: the service HEADs the stored object and writes the size and the kind storage holds, and a file outside the caps leaves the bucket again. A delete removes the row, then the object. No money moves.
...
- Money on screen goes through `@repo/config/money` (`usd`, `usdCents`, `usdPerThousand`, `parseUsd`). No page divides by the peg
...
- "Run the database tests" → `pnpm db:up`, then `pnpm test:db`. The `*.db.test.ts` files under `apps/api` run against `TEST_DATABASE_URL` (default `…/adstukar_test`), which the global setup creates and migrates. `pnpm verify` never runs them
...
- **Bought money never expires.** The expiry job must skip the `bought` lot. Expiring money somebody paid for is a consumer-law problem.
...
- The gate must keep passing with **no `.env` present** — that is what CI and a fresh clone get. A test that needs Postgres is a `*.db.test.ts` file under `apps/api`; `pnpm test:db` runs those against a `_test` database, and CI runs it as its own step after `pnpm verify` with a postgres service. Never fold a database test into `pnpm verify`.
