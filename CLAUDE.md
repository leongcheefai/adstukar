...
- **Slots** — `apps/api/src/modules/slots/`: `slots.ts` holds the pure rules (the term, what refunds, the loop, availability), `term.ts` starts, refunds and ends a term inside the transaction of the act that moved it, and `slots.service.ts` books in one transaction: campaign, creative, charge, slot. The charge is a `spend` with no `playId`, so the daily budget never sees it. Only `booked` refunds. `GET /slots/loop` is public and names no member. See `docs/adr/0009`.
