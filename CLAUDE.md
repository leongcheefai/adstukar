...
- **Jobs** — `apps/api/src/modules/jobs/`: settlement (pending → settled), expiry, and the void of an open play whose report never came. A fourth job paces campaigns. It stops a campaign that can no longer pay, and starts one whose reason to stop has gone. A fifth job ends a slot whose term is over. The jobs run in-process (`JOBS_ENABLED=true`), or once with `pnpm --filter @repo/api jobs:run`.
